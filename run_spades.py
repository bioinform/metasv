...
def append_contigs(src, interval, dst_fd, fn_id = 0, sv_type = "INS"):
  with open(src, "r") as fd:
    for line in fd:
      if line.startswith(">"):
        dst_fd.write(">%s_%d_%d_%s_%d_%s" % (interval.chrom, interval.start, interval.end, sv_type, fn_id, line[1:]))
      else:
        dst_fd.write(line)

def run_spades_single(intervals = [], bam = None, spades = None, work = None, pad = 0, myid = 0, timeout = 300, isize_min = 300, isize_max = 500):
  ...
  extract_fns = [extract_pairs.all_pair, extract_pairs.non_perfect, partial(extract_pairs.discordant, isize_min = isize_min, isize_max = isize_max), partial(extract_pairs.discordant_with_normal_orientation, isize_min = isize_min, isize_max = isize_max)]
  update_wrapper(extract_fns[2], extract_pairs.discordant) # wrap to make sure the partial object has __name__ attribute
  ...
  for interval in intervals:
    ...
    for fn_id, ((end1, end2), extracted_count) in enumerate(extract_pairs.extract_read_pairs(bam, region, "%s/" % (work), extract_fns, pad = pad)):
      extract_fn_name = extract_fns[fn_id].__name__
      if extracted_count >= 5:
        extra_opt = "--sc" if not fn_id == 0 else ""
        retcode = run_cmd("bash -c \"timeout %ds %s -1 %s -2 %s -o %s/spades_%s/ -m 4 -t 1 %s\"" % (timeout, spades, end1, end2, work, extract_fn_name, extra_opt), thread_logger, spades_log_fd)
        if retcode == 0:
          append_contigs(os.path.join(work, "spades_%s/contigs.fasta") % (extract_fn_name), interval, merged_contigs, fn_id, sv_type)
      else:
...
def run_spades_parallel(bam = None, spades = None, bed = None, work = None, pad = 0, nthreads = 1, chrs = [], max_interval_size = 50000, timeout = 300, isize_min = 250, isize_max = 450):
  ...
  chrs = set(chrs)
  all_intervals = [interval for interval in bedtool] if not chrs else [interval for interval in bedtool if interval.chrom in chrs]
  ...
  for i in xrange(nthreads):
    ...
    kwargs_dict = {"intervals": intervals, "bam": bam, "spades": spades, "work": "%s/%d" % (work, i), "pad": pad, "myid": i, "timeout": timeout, "isize_min": isize_min, "isize_max": isize_max}
    pool.apply_async(run_spades_single, kwds = kwargs_dict, callback = partial(run_spades_single_callback, result_list = assembly_fastas))
...
if __name__ == "__main__":
  ...
  run_spades_parallel(bam = args.bam, spades = args.spades, bed = args.bed, work = args.work, pad = args.pad, nthreads = args.nthreads, chrs = args.chrs, max_interval_size = args.max_interval_size, isize_min = args.isize_min, isize_max = args.isize_max)
