...
class BreakDancerRecord:
  ...
  def to_sv_interval(self):
    ...
    if self.sv_type == "DEL":
      return SVInterval(self.chr1, self.pos1, self.pos1 + abs(self.sv_len), sv_type=self.sv_type, length=self.sv_len, sources=breakdancer_source, cipos=[0, self.pos2 - self.pos1 - abs(self.sv_len)], native_sv=self)
    else:
