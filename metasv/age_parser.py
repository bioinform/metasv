...
class AgeRecord:
    ...
    with open(age_out_file) as age_fd:
      while True:
        ...
        if line.startswith("Alignment:"):
          self.start1_end1s = map(lambda y: map(int, y), map(lambda x: x.split(","), line.split(":")[1].split()))
          self.start2_end2s = map(lambda y: map(int, y), map(lambda x: x.split(","), line.split(":")[2].split()))

          self.nfrags = len(self.start1_end1s)
          self.flanking_regions[0] = abs(self.start2_end2s[0][1] - self.start2_end2s[0][0] + 1)
          self.flanking_regions[1] = 0 if len(self.start2_end2s) == 1 else abs(self.start2_end2s[1][1] - self.start2_end2s[1][0] + 1)

          self.ref_flanking_regions[0] = abs(self.start1_end1s[0][1] - self.start1_end1s[0][0] + 1)
          self.ref_flanking_regions[1] = 0 if len(self.start1_end1s) == 1 else abs(self.start1_end1s[1][1] - self.start1_end1s[1][0] + 1)
          if len(self.start2_end2s) > 1:
            cs, ce = self.start1_end1s[0][1], self.start2_end2s[1][0]
          continue

        if line.startswith("EXCISED REGION(S):"):
          self.excised_regions = map(lambda y: map(int, y), map(lambda x: x.split(","), line.split(":")[1].split()))
          continue
        ...
        if line.startswith("Identity at b"):
          self.breakpoint_identities = map(lambda y: map(int, y), map(lambda x: x.split(","), line.split(":")[1].split()))
  ...
  def has_insertion(self, min_diff = 1, max_diff = 49, window = 20):
    return len(self.start2_end2s) == 2 and min_diff <= abs(self.start2_end2s[1][0] - self.start2_end2s[0][1] - 1) <= max_diff and abs(self.start1_end1s[1][0] - self.start1_end1s[0][1] - 1) <= window
  ...
  def has_long_flanks(self, min_len):
    return self.flanking_regions[0] >= min_len and self.flanking_regions[1] >= min_len + self.hom and min(self.ref_flanking_regions[0], self.ref_flanking_regions[1]) >= min_len

  def has_only_long_left_flank(self, min_len):
    return min(self.ref_flanking_regions[0], self.flanking_regions[0]) >= min_len and self.flanking_regions[1] <= min_len

  def has_only_long_right_flank(self, min_len):
    return self.flanking_regions[0] <= min_len and self.flanking_regions[1] >= min_len + self.hom and self.ref_flanking_regions[1] >= min_len
  ...
  def is_reference(self, min_flank_length = 100, max_length_diff = 20):
    if len(self.start1_end1s) == 1:
      if abs(self.start1_end1s[0][1] - self.start1_end1s[0][0] + 1) >= self.contig.sv_region.length() - 100: return True
      return False
