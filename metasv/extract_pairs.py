...
def keep_read(aln,aln_chr, chromosome, start, end):
   return aln_chr == chromosome and (not is_clipped_both(aln)) and aln.mapq >= 40 and aln.pos >= start and aln.pos < end
...
def is_all(aln,mate):
  return ((aln.cigarstring == '100M' and mate.cigarstring == '100M') and ( aln.is_proper_pair and mate.is_proper_pair))
...
def extract_read_pairs(bamname, region, prefix, extract_fns, pad = 0):
  ...
  logger.info("Extracting reads from %s for region %s with padding %d using functions %s" % (bamname, region, pad, extract_fn_names))
  ...
  ends = [(open("%s_%s_1.fq" % (prefix, name), "w"), open("%s_%s_2.fq" % (prefix, name), "w")) for name in extract_fn_names]
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser("Extract reads and mates from a region for spades assembly", formatter_class = argparse.ArgumentDefaultsHelpFormatter)
  ...
  parser.add_argument("--extract_fn", help = "Extraction function", choices = ["all_pair", "non_perfect", "discordant"], default = "all_pair")
