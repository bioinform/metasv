...
class TigraContig:
  ...
  def parse_name(self, name):
    ...
    if len(name_fields) == 4:
      ...
      if name_fields[4][1:]:
        self.contig_i = map(lambda y: (y[0], int(y[1])),  map(lambda x: x.split(":"), name_fields[4][1:].split(",")[:-1]))

      if name_fields[5][1:]:
        self.contig_o = map(lambda y: (y[0], int(y[1])),  map(lambda x: x.split(":"), name_fields[5][1:].split(",")[:-1]))
