...
def generate_sc_intervals(bam, chromosome, workdir, min_avg_base_qual = 20, min_mapq = 5, min_soft_clip = 20, max_soft_clip = 50, pad = 500, min_support = 5, max_isize = 1000000000):
  ...
  for aln in sam_file.fetch(reference = chromosome):
    ...
    if not is_good_candidate(aln, min_avg_base_qual = min_avg_base_qual, min_mapq = min_mapq, min_soft_clip = min_soft_clip, max_soft_clip = max_soft_clip): continue
    interval = get_interval(aln, pad = pad)
    soft_clip_location = sum(interval) / 2
    strand = "-" if aln.is_reverse else "+"
    name = "%d,%s" % (soft_clip_location, strand)
    unmerged_intervals.append(pybedtools.Interval(chromosome, interval[0], interval[1], name = name, score = "1", strand = strand))
  sam_file.close()
...
def parallel_generate_sc_intervals(bams, chromosomes, skip_bed, workdir, num_threads = 1, min_avg_base_qual = 20, min_mapq = 5, min_soft_clip = 20, max_soft_clip = 50, pad = 500, min_support = 2):
  func_logger = logging.getLogger("%s-%s" % (parallel_generate_sc_intervals.__name__, multiprocessing.current_process()))
  ...
  for index, (bam, chromosome) in enumerate(itertools.product(bams, chromosomes)):
    ...
    kwargs_dict = {"min_avg_base_qual": min_avg_base_qual, "min_mapq": min_mapq, "min_soft_clip": min_soft_clip, "max_soft_clip": max_soft_clip, "pad": pad, "min_support": min_support}
    pool.apply_async(generate_sc_intervals, args = args_list, kwds = kwargs_dict, callback = partial(generate_sc_intervals_callback, result_list = bed_files))
  ...
  if skip_bed:
    ...
    func_logger.info("Merging %d features with %d features from %s" % (bedtool.count(), skip_bedtool.count(), skip_bed))
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(description="Generate BED intervals for insertion detection using soft-clipped reads", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  ...
  parallel_generate_sc_intervals(args.bams, args.chromosomes, args.skip_bed, args.workdir, num_threads = args.num_threads, min_avg_base_qual = args.min_avg_base_qual, min_mapq = args.min_mapq, min_soft_clip = args.min_soft_clip, max_soft_clip = args.max_soft_clip, pad = args.pad, min_support = args.min_support)
