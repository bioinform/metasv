...
def run_age_single(intervals_bed = None, region_list = [], contig_dict = {}, reference = None, assembly = None, pad = 500, age = None, age_workdir = None, timeout = 300, keep_temp = False, myid = 0):
  ...
  try:
    for region in region_list:
      ...
      matching_intervals = [interval for interval in intervals_bedtool if (interval.start == bedtools_interval.start and interval.end == bedtools_interval.end and interval.chrom == bedtools_interval.chrom)]
      ...
      reference_sequence = reference_fasta.fetch(reference = region_object.chrom1, start = region_object.pos1 - pad, end = region_object.pos2 + pad)
      ...
      for contig in contig_dict[region]:
        thread_logger.info("Writing the assembeled sequence %s of length %s" % (contig.raw_name, contig.sequence_len))
        ...
        with open(asm_name, "w") as file_handle: file_handle.write(">{}.as\n{}".format(region_name, contig_sequence))

        age_cmd = "%s %s -both -go=-6 %s %s >%s 2>%s" % (age, "-inv" if contig.sv_type == "INV" else "-indel", ref_name, asm_name, out, err)
...
def run_age_parallel(intervals_bed = None, reference = None, assembly = None, pad = 500, age = None, age_workdir = None, timeout = 300, keep_temp = False, assembly_tool = "spades", chrs = [], nthreads = 1, min_contig_len = 200, max_region_len = 50000, sv_types = []):
  ...
  contig_dict = {contig.sv_region.to_tuple(): [] for contig in contigs if (len(chrs) == 0 or contig.sv_region.chrom1 in chrs) and contig.sequence_len >= min_contig_len and contig.sv_region.length() <= max_region_len and (len(sv_types) == 0 or contig.sv_type in sv_types)}
  ...
  func_logger.info("Will process %d regions with %d contigs (%d small contigs ignored) using %d threads" % (len(region_list), sum([len(value) for value in contig_dict.values()]), small_contigs_count, nthreads))
  ...
  for i in xrange(nthreads):
    ...
    kwargs_dict = {"intervals_bed": intervals_bed, "region_list": region_sublist, "contig_dict": contig_dict, "reference": reference, "assembly": assembly, "pad": pad, "age": age, "age_workdir": age_workdir, "timeout": timeout, "keep_temp": keep_temp, "myid": i}
    pool.apply_async(run_age_single, args = [], kwds = kwargs_dict, callback = partial(run_age_single_callback, result_list = breakpoints_beds))
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description = "Run AGE on files assembled under MetaSV.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  ...
  run_age_parallel(intervals_bed = args.intervals_bed.name, reference = args.reference.name, assembly = args.assembly.name, pad = args.pad, age = args.age.name, age_workdir = args.work, timeout = args.timeout, keep_temp = args.keep_temp, assembly_tool = args.assembly_tool, chrs = args.chrs, nthreads = args.nthreads, min_contig_len = args.min_contig_len, max_region_len = args.max_region_len, sv_types = args.sv_types)
