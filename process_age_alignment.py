...
def pair_intervals(intervals, reference_length, min_interval_length = 200, window = 50):
  ...
  for interval1 in intervals:
    ...
    for interval2 in intervals:
      ...
      overlap_interval = (max(interval1_fixed[0], interval2_fixed[0]), min(interval1_fixed[1], interval2_fixed[1]))
      overlap_interval = (min(overlap_interval), max(overlap_interval))

      if overlap_interval[1] - overlap_interval[0] <= 20 and overlap_interval[0] >= window and overlap_interval[1] <= window + reference_length:
...
def get_insertion_breakpoints(age_records, intervals, window = 20, sv_type = "INS", start = 0, pad = 500):
  ...
  potential_breakpoints = sorted([interval.start for interval in bedtools_intervals] + [interval.end for interval in bedtools_intervals])
  ...
  for breakpoint in potential_breakpoints[1:-1]:
    ...
    if (left_support and right_support) and min([window + 1] + [abs(b[0] - breakpoint) for b in breakpoints]) > window:
      both_support = [age_record for age_record in age_records if age_record.has_insertion(min_diff = 50, max_diff = 1000000000) and age_record.breakpoint_match(breakpoint, window)]
...
def get_deletion_breakpoints(age_records, window = 20, min_flank_length = 50, start = 0):
  ...
  potential_breakpoints = sorted([age_record.start1_end1s[0][1] for age_record in age_records] + [age_record.start1_end1s[1][0] for age_record in age_records])
  breakpoints = []
  for breakpoint in potential_breakpoints:
    left_support = [age_record for age_record in age_records if abs(age_record.start1_end1s[0][1] - breakpoint) < window]
    right_support = [age_record for age_record in age_records if abs(age_record.start1_end1s[1][0] - breakpoint) < window]
...
def generate_assembly_features(assemblies):
  ...
  return map(str, [len(assemblies), len(unique_assemblies), int(len(assemblies) > 0), is_consistent, best_assembly.insertion_length, best_assembly.start, best_assembly.end, best_assembly.excised_lengths[0], best_assembly.excised_lengths[1]])
...
def process_age_output(tigra_contig, age_file, window, min_id = 95, min_contig_len = 100):
  ...
  if age_record.aligned_bases + 10 >= tigra_contig.sequence_len:
    logger.info("Very bad assembly: too many aligned bases %d of %d" % (age_record.aligned_bases, tigra_contig.sequence_len))
    age_record.very_bad_assembly = True
    return None
  ...
  logger.info("Success: %s %d %d %d %d %d %d %d" % (tigra_contig.sv_region.chrom1, tigra_contig.sv_region.pos1, tigra_contig.sv_region.pos2, s, e, age_record.hom, age_record.cs, age_record.ce))
  return age_record
...
def get_reference_intervals(age_records, start = 0, min_interval_len = 100):
  ...
  for age_record in age_records:
    intervals += map(lambda x: (min(x) + start - 1, max(x) + start - 1), [interval for interval in age_record.start1_end1s if abs(interval[0] - interval[1]) >= min_interval_len])

  return intervals

def process_age_records(age_records, sv_type = "INS", ins_min_unaligned = 10, min_interval_len = 200, pad = 500, min_deletion_len = 30):
  ...
  if sv_type == "INS":
    good_age_records = [age_record for age_record in good_age_records if not age_record.almost_all_bases_aligned(ins_min_unaligned)]
    good_age_records = [age_record for age_record in good_age_records if not age_record.is_reference()]
  elif sv_type == "DEL":
    good_age_records = [age_record for age_record in good_age_records if len(age_record.start1_end1s) == 2 and min(age_record.ref_flanking_regions) >= 50]
    good_age_records = [age_record for age_record in good_age_records if abs(age_record.start1_end1s[0][1] - age_record.start1_end1s[1][0]) >= min_deletion_len]
    good_age_records = [age_record for age_record in good_age_records if float(age_record.score) / sum(age_record.ref_flanking_regions) >= 0.7]
  elif sv_type == "INV":
    pass
